<!-- Title -->
<header>
  <div class=" flex flex-col  mx-8 mt-20 font-serif">
    <h1 class=" text-5xl md:text-6xl lg:text-7xl font-semibold text-red-400">
      Why Prota?
    </h1>
    <p class=" text-lg lg:text-xl my-2 ">
      I’m a creative soul, specialized in Branding and UI/UX design. I always
      focus on the user experience first.
    </p>
  </div>
</header>

<div class=" flex flex-col md:grid md:grid-cols-3 md:mt-8 md:gap-8 lg:gap-0 ">
  <img class=" object-cover h-80 mx-8 my-2" src="/assets/img8.png" alt="Me" />

  <div class=" md:col-span-2">
    <!-- Description -->
    <div
      class=" flex flex-col md:flex-row md:justify-start md:items-center lg:gap-x-16 mx-8 my-8 font-serif"
    >
      <div>
        <h1 class=" text-3xl font-semibold text-gray-900">
          What can i help you with?
        </h1>
        <p class=" text-lg my-2 ">
          If you need help with the design of an app or a website, or if you
          need a visual identity for your business, i will use my knoledge in
          these areas to help you connect with your target audience through my
          process.
        </p>
      </div>
    </div>

    <!-- Contact link -->
    <div class=" flex flex-col  mx-8 my-8 font-serif">
      <h1 class=" text-3xl font-semibold text-gray-900">
        Tell me about your project
      </h1>
      <a href="/contact" class=" flex my-2 text-red-400 hover:text-red-600">
        <h2 class=" text-xl font-semibold ">If you are interested :)</h2>
        <svg
          class="fill-current h-8 w-6 ml-1 "
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M18.17 13L15.59 15.59L17 17L22 12L17 7L15.59 8.41L18.17 11H2V13H18.17Z"
          />
        </svg>
      </a>
    </div>
  </div>
</div>
